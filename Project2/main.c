...
int main(void)
{
    ...
    while (1) {
        // Ana döngüde hiçbir işlem yapılmıyor
    }
}
...
void timerkesmefonksiyonu()
{
    ...
    char bb = (char)dakika;
    char cc = (char)saniye;
    UARTCharPut(UART0_BASE, aa);
    UARTCharPut(UART0_BASE, bb);
    UARTCharPut(UART0_BASE, cc);

    // Zamanı formatla ve LCD'ye yazdır
    char buffer[9];
    format_time(buffer, saat, dakika, saniye);
    lcd_command(0xC8); // 2. satıra git
    lcd_print(buffer);

    // PF2 LED'in durumunu değiştir (görsel test için)
...
void serikesme()
{
    ...
    //GPIOPinWrite(GPIO_PORTB_BASE, 255, saniye);
    //UARTIntDisable(UART0_BASE, UART_INT_RX|UART_INT_RT);
}
